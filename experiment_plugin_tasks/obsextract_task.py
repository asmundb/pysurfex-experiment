...
class ObsExtract(AbstractTask):
    ...
    def execute(self):
        ...
        diag_file = self.platform.substitute("SURFOUT.@YYYY@@MM_LL@@DD_LL@_@HH_LL@h00.nc",basetime=self.dtg, validtime=self.dtg + self.fcint)
        print("diag_file", diag_file)
        obpattern = self.config.get_value("assim.hofxpath")
        obpattern = self.platform.substitute(obpattern, basetime=self.dtg + self.fcint)
